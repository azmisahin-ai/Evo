...
class OutputChannel:
    ...
    def send(self, output_data):
        """Çıktı verisini bu kanala gönderir. Alt sınıflar implement etmelidir."""
        # logging.debug(f"OutputChannel '{self.name}': send metodu çağrildi.")
...
class ConsoleOutputChannel(OutputChannel):
    ...
    def __init__(self, config=None):
        ...
        logging.info(f"ConsoleOutputChannel başlatıldı.") # Daha spesifik log

    def send(self, output_data):
        """
        Çıktı verisini alır. Bu kanalda, çıktı Ana döngüde InteractionAPI
        tarafından INFO seviyesinde loglandığı için burada ek bir loglama yapılmaz.
        Gelecekte farklı konsol formatları veya renkler eklenebilir.
        """
        if output_data is not None:
             logging.debug(f"OutputChannel '{self.name}': Ham çıktı alindi, isleniyor/hazirlaniyor (Konsol).")
             # Şimdilik çıktı verisini doğrudan loglamıyor, çünkü InteractionAPI.send_output bunu yapacak.
             # Eğer burası farklı bir formatta konsola yazacaksa (örn. renklendirme), o mantık buraya gelir.
...
class WebAPIOutputChannel(OutputChannel):
    ...
    def __init__(self, config=None):
        ...
        logging.info(f"WebAPIOutputChannel başlatıldı.") # Daha spesifik log
...
class InteractionAPI:
    ...
    def __init__(self, config=None):
        ...
        logging.info(f"InteractionAPI: Konfigurasyondan aktif kanallar: {enabled_channels}")
    ...
    def send_output(self, output_data):
        """
        MotorControl modülünden gelen çıktıyı alır.
        Çıktıyı ana log akışına yazar (INFO seviyesi) ve tüm aktif kanallara gönderir.
        ...
        # Çıktıyı Ana Log Akışına Yaz (INFO seviyesi)
        logging.info(f"Evo Çıktısı: {output_data}") # Bu satır run_evo.py'den buraya taşındı.
        ...
        for channel_name, channel_obj in self.output_channels.items():
            try:
                logging.debug(f"InteractionAPI: Çikti OutputChannel '{channel_name}' kanalına gönderiliyor.") # DEBUG logu
...
if __name__ == '__main__':
    ...
    test_config = {
        'enabled_channels': ['console', 'web_api'], # web_api placeholder, console gerçek
        'channel_configs': {
    ...
    try:
        ...
        api.send_output(dummy_output_1) # Bu çağrı INFO logunu tetikleyecek

        print("\nÇikti 'Bu bir test mesajı.' kanallara gönderiliyor:")
        api.send_output(dummy_output_2) # Bu çağrı INFO logunu tetikleyecek

        # None çıktı test et
        print("\nNone çikti kanallara gönderiliyor:")
        api.send_output(None) # None ise InteractionAPI.send_output'a hiç girmez

        # Boş string çıktı test et
        print("\nBoş string çikti kanallara gönderiliyor:")
        api.send_output("") # Boş string gönderilebilir, INFO logu boş satır yazacak
